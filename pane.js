export class Pane {
    constructor(context, output_node, container, on_change) {
        ...
        if (this.output_node != null) {
            ...
            svg.addEventListener("touchstart", (e) => {
                this.dragging = true;
                this.handle_mouse(e.changedTouches.item(0));
            }, { passive: true });
    ...
    from_svg(clientX, clientY) {
        const rect = this.svg.getBoundingClientRect();
        return [
            Math.max(0, Math.min(Math.round(((clientX - rect.left) / rect.width) * this.samples.length), this.samples.length - 1)),
